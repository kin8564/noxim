...
extract_metrics() {
    ...
    if [[ -z "${throughput}" || -z "${delay}" || -z "${energy}" ]]; then
        return 1
    fi
...
echo "Channels,PIR,Run,Seed,Throughput_flits_cycle_IP,Global_Avg_Delay_cycles,Energy_J" > "${RAW_CSV}"
echo "Channels,PIR,Avg_Throughput_flits_cycle_IP,Avg_Global_Avg_Delay_cycles,Avg_Energy_J,Valid_Runs" > "${AVG_CSV}"
...
for channels in ${CHANNEL_VALUES}; do
    ...
    for pir in ${PIR_VALUES}; do
        ...
        for run in $(seq 0 $((NUM_RUNS - 1))); do
            ...
            IFS='|' read -r thr dly eng <<< "${metrics}"
            printf "OK thr=%.6e delay=%.2f energy=%.3e\n" "${thr}" "${dly}" "${eng}"
            echo "${channels},${pir},$((run + 1)),${seed},${thr},${dly},${eng}" >> "${RAW_CSV}"
        done

        awk -F',' -v c="${channels}" -v p="${pir}" '
            BEGIN { t=0; d=0; e=0; n=0 }
            NR > 1 && $1 == c && $2 == p {
                t += $5; d += $6; e += $7;
                n += 1;
            }
...
echo "Top channel count per PIR (min Avg_Energy):"
awk -F',' '
    NR == 1 { next }
    {
        pir = $2;
        ...
        if (!(pir in best_energy) || en < best_energy[pir]) {
            best_energy[pir] = en;
    ...
    END {
        printf "PIR,Best_Channels,Avg_Throughput,Avg_Global_Avg_Delay,Avg_Energy\n";
        for (pir in best_channels) {
            printf "%s,%s,%.10e,%.6f,%.10e\n", pir, best_channels[pir], best_thr[pir], best_delay[pir], best_energy[pir];
        }
    }
' "${AVG_CSV}"
