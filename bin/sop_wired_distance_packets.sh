...
RESULTS_FILE="${RESULTS_DIR}/sop_wired_distance_packets_${TIMESTAMP}.csv"
AVG_RESULTS_FILE="${RESULTS_DIR}/sop_wired_distance_packets_avg_${TIMESTAMP}.csv"
...
for vc in "${VC_VALUES[@]}"; do
    ...
    for pir in "${PIR_VALUES[@]}"; do
        ...
        for run in $(seq 0 $((NUM_RUNS-1))); do
            ...
            output=$(timeout 120 ./noxim -config "$config_file" -vc "$vc" -traffic longdist -pir "$pir" poisson -seed "$seed" 2>&1)
