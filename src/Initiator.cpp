...
void Initiator::thread_process()
{
	...
	tlm::tlm_phase phase;
	sc_time delay;
	const unsigned int max_backoff_exponent = 6;
	...
	while (1)
	{
		...
		while (!transmission_completed)
		{
			...
			{
				if (GlobalParams::use_wireless_backoff)
				{
					unsigned int backoff_exponent = retry_attempt < max_backoff_exponent ? retry_attempt : max_backoff_exponent;
					unsigned int backoff_cycles = 1u << backoff_exponent;
					LOG << " WARNING: incomplete transaction, backing off for " << backoff_cycles
					    << " cycles (attempt " << (retry_attempt + 1) << ")" << endl;
					if (retry_attempt < max_backoff_exponent)
						retry_attempt++;
					wait(sc_time(backoff_cycles * GlobalParams::clock_period_ps, SC_PS));
				}
				else
				{
